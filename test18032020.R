# Einleitung durch Martin am 18.03.2020
# Code bereitgestellt durch Martin per Mail vom 18.03.
...
for(year in 1986:2013){
  ...
  X <- readRDS(file=paste0("/mnt/nfs_fineprint/tmp/fabio/",year,"_X.rds")) # output, zeilensumme über Y und Z
  ...
  ext <- as.vector(E$Landuse) / X # wählt die entsprechende Kategorie aus E-Variable
  # => land-use pro Country and Item (e.g. ha/Weizen Afghanistan
  # X = Tonnen => E/X => ha/ton)
  
  ext[!is.finite(ext)] <- 0 # setzt unendliche ergebnisse gleich 0
